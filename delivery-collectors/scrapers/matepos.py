...
# DJANGO_URL   = os.getenv("DJANGO_URL", "http://localhost:8000/api/sales/bulk-create/")

HQ_BRAND_ID        = os.getenv("HQ_BRAND_ID")
MS_STR_ID          = os.getenv("MS_STR_ID")
STORE_CODE         = os.getenv("STORE_CODE", MS_STR_ID)
...
enpCd = os.getenv("ENPCD")
corpCd = os.getenv("CORPCD")
brandCd = os.getenv("BRANDCD")
...
async def push_to_django(client: httpx.AsyncClient, records: list[dict]) -> dict:
    resp = await client.post(SALES_BULK_CREATE, json=records, timeout=60)
    resp.raise_for_status()
    return resp.json()
