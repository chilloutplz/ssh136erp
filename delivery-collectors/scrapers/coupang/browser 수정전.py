...
async def _is_session_valid(page: Page) -> bool:
    try:
        await page.goto(ORDER_PAGE_URL, wait_until="domcontentloaded", timeout=15000)
        ...
        try:
            ...
            if await close_btn.is_visible(timeout=5000):
                print("[coupang][browser] 공지 팝업 감지")

                await close_btn.click(force=True)

                await asyncio.sleep(1)

                print("[coupang][browser] 공지 팝업 닫기 완료")

        except Exception as e:
            print(f"[coupang][browser] 팝업 없음 또는 닫기 실패: {e}")
